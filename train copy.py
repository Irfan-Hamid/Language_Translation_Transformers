...
def greedy_decode_whole(model, source, source_mask, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        # Use causal mask for initial decoding to get the next token prediction
        decoder_mask_initial = causal_mask(decoder_input.size(1)).type_as(source_mask).to(device)
        out_initial = model.decode(encoder_output, source_mask, decoder_input, decoder_mask_initial)
        prob_initial = model.project(out_initial[:, -1])
        _, next_word_initial = torch.max(prob_initial, dim=1)

        # Append the predicted token to the decoder input
        decoder_input = torch.cat(
            [decoder_input, torch.empty(1, 1).type_as(source).fill_(next_word_initial.item()).to(device)], dim=1
        )

        # Use causal mask with future for refining the next token prediction
        decoder_mask_future = causal_mask_with_future(decoder_input.size(1)).type_as(source_mask).to(device)
        out_future = model.decode(encoder_output, source_mask, decoder_input, decoder_mask_future)
        prob_future = model.project(out_future[:, -1])
        _, next_word_future = torch.max(prob_future, dim=1)

        # Replace the last token in the decoder input with the refined prediction
        decoder_input[:, -1] = torch.tensor([next_word_future.item()], device=device)

        if next_word_future == eos_idx:
            break
